    ...
    <div className="w-full overflow-x-hidden relative" style={{ background: '#030712' }}>
      ...
      <section className="relative py-28 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 right-0 -translate-y-1/2 w-[600px] h-[600px] rounded-full"
            style={{ background: 'radial-gradient(circle 60% at 60% 0%, rgba(139,92,246,0.06), transparent 70%)' }} />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-wrap items-center gap-2 text-xs text-[#64748B] mb-8">
            ...
            <ChevronRight className="w-3 h-3" />
            <Link href={`/services/${catData.slug}`} className="hover:text-[#E2E8F0] transition-colors">{catData.name}</Link>
            <ChevronRight className="w-3 h-3" />
            <span className="text-[#06B6D4] font-medium">{svcData.name}</span>
          </div>

          <div className="max-w-4xl">
            <span className="text-5xl mb-6 block">{svcData.icon}</span>
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#06B6D4] mb-3">{svcData.tagline}</p>
            <h1 className="text-5xl md:text-6xl font-black text-[#E2E8F0] mb-6 leading-tight">{svcData.name}</h1>
            <p className="text-xl text-[#64748B] leading-relaxed mb-10 max-w-2xl">{svcData.description}</p>
            <Link href="/contact"
      ...
      <section className="py-24" style={{ background: '#030712', borderTop: '1px solid #1E293B' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#06B6D4] mb-3">Capabilities</p>
              <h2 className="text-4xl font-black text-[#E2E8F0] mb-6">What We Will Deliver</h2>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {svcData.features.map((feat) => (
                <div key={feat} className="flex items-start gap-3 p-5 rounded-2xl"
                  ...
                  <span className="text-[#E2E8F0] font-medium text-sm">{feat}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-24 relative overflow-hidden" style={{ background: '#030712', borderTop: '1px solid #1E293B' }}>
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-4xl md:text-5xl font-black text-[#E2E8F0] mb-6">Ready to scale your business with {svcData.name}?</h2>
          <p className="text-[#64748B] mb-10 text-lg">Book a free strategy session with our experts today. No commitments, just clear actionable guidance.</p>
