...
function ClientCard({ c }: { c: { name: string; industry: string; color: string; logo?: string } }) {
  return (
    <div
    ...
    >
      <div
        className="flex items-center justify-center font-black text-xl overflow-hidden"
        style={{
          width: 52, height: 52,
          borderRadius: 14,
      ...
      >
        {c.logo
          ? <img src={c.logo} alt={c.name} style={{ width: 38, height: 38, objectFit: 'contain', borderRadius: 8 }} />
          : c.name[0]
        }
      </div>
      <div>
...
/* ─── CMS types ─────────────────────────────────────────────── */
export interface HeroCMS { badge:string; headline1:string; headline2:string; headline3:string; subheadline:string; cta1Text:string; cta1Href:string; cta2Text:string; cta2Href:string; stat1Value:string; stat1Label:string; stat2Value:string; stat2Label:string; stat3Value:string; stat3Label:string; stat4Value:string; stat4Label:string; }

interface Props {
  cmsHero: HeroCMS;
  cmsClients: { name: string; industry: string; color: string; logo?: string }[];
  cmsTestimonials: { quote: string; name: string; title: string; metric: string; metricLabel: string; stars?: number }[];
  cmsWhyUs: { Icon?: React.ElementType; title: string; desc: string }[];
  cmsSteps: { num: string; title: string; desc: string }[];
}

/* ─── Component ─────────────────────────────────────────────── */

export default function HomeClient({ cmsHero, cmsClients, cmsTestimonials, cmsWhyUs, cmsSteps }: Props) {
  ...
  const hero          = cmsHero;
  const clientsList   = cmsClients;
  const testimonialsList = cmsTestimonials;
  const whyUsList     = cmsWhyUs;
  const stepsList     = cmsSteps;
      ...
      <section className="relative overflow-hidden flex flex-col" style={{ background: '#050B14', height: '100vh', paddingTop: '80px' }}>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 text-center">
          ...
          <FadeIn delay={0.07}>
            <h1 className="mb-5"
              ...
              <span style={{ color: 'rgba(255,255,255,0.9)', display: 'block' }}>{hero.headline1}</span>
              <span style={{
                display: 'block',
              ...
              <span style={{ color: 'rgba(255,255,255,0.85)', display: 'block' }}>{hero.headline3}</span>
            </h1>
          </FadeIn>
        ...
        <div className="relative z-10 pb-10 px-4">
          <div className="max-w-2xl mx-auto flex items-center justify-center"
            ...
            {[[hero.stat1Value,hero.stat1Label],[hero.stat2Value,hero.stat2Label],[hero.stat3Value,hero.stat3Label],[hero.stat4Value,hero.stat4Label]].map(([val,lbl], i) => (
              <div key={i} className="flex-1 flex flex-col items-center py-5"
